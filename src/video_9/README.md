...
Source code for the YouTube video on using Kùzu and LangChain for simple Graph RAG.
...
This will build a graph from the unstructured text data using LangChain's `LLMGraphTransformer`
and store it in a Kùzu database, and then use `KuzuQAChain`to query the graph via an LLM to respond
to the question in natural language.
